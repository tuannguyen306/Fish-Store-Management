class Fish < ApplicationRecord
  ...
  validates :water_type, inclusion: { in: %w[freshwater saltwater brackish], case_sensitive: false }
  ...
  def image_content_type_validation
    if image.attached? && !image.content_type.in?(%('image/png image/jpg image/jpeg image/gif'))
      ...
      image.purge # Remove invalid file
      throw(:abort) # Prevent further validation
    end
  end
